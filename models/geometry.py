...
class MarchingCubeHelper(nn.Module):
    def __init__(self, sdf_func, bounds, resolution, block_res=256, method="mc"):
        ...
        try:
            ...
            print(
                "Cannot find cuda accelerated marching cube, downgraded to cpu version!"
            )
            self.method = "mc"
    ...
    def forward_(self, level, threshold=0.0):
        if self.method == "CuMCubes":
            ...
            verts, faces = self.mc_func(
                -level.cpu().numpy(), threshold
            )  # transform to numpy
            verts, faces = torch.from_numpy(verts.astype(np.float32)), torch.from_numpy(
                faces.astype(np.int64)
            )  # transform back to pytorch
        return verts, faces

    def forward(self, threshold=0.0):
        mesh_blocks = []
    ...
    def __init__(self, config):
        ...
        if self.config.isosurface is not None:
            ...
            self.helper = MarchingCubeHelper(
                self.sdf_func,
                self.bounds,
                int(self.config.isosurface.resolution),
                method=self.config.isosurface.method,
            )
...
class VolumeSDF(BaseImplicitGeometry):
    ...
    def update_step(self, epoch, global_step):
        ...
        if isinstance(self.finite_difference_eps, float):
            ...
            ), "finite_difference_eps='progressive' only works with ProgressiveBandHashGrid"
            current_level = min(
                hg_conf.start_level
            ...
            raise ValueError(
                f"Unknown finite_difference_eps={self.finite_difference_eps}"
            )
