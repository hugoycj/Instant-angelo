...
class Herzjesu(data.Dataset):
    ...
    def __getitem__(self, i):
        ...
        ref_cam, *srcs_cam = [os.path.join(self.root, f'cams_t/{idx:08}_cam.txt') for idx in [ref_idx] + src_idxs]
        skip = 0
...
def get_val_loader(root, num_src, preproc_args):
    def channel_first_wrapper(sample):
        ref, ref_cam, srcs, srcs_cam, gt, masks = sample
        ref, *srcs = to_channel_first([ref] + srcs)
        return ref, ref_cam, srcs, srcs_cam, gt, masks
    dataset = Herzjesu(
        root, num_src,
